...
echo "$($PSQL "SELECT SUM(CAST(winner_goals AS DECIMAL))/COUNT(*) from games;")"

echo -e "\nAverage number of goals in all games from the winning teams rounded to two decimal places:"
echo "$($PSQL "SELECT ROUND(SUM(CAST(winner_goals AS DECIMAL))/COUNT(*), 2) from games;")"

echo -e "\nAverage number of goals in all games from both teams:"
echo "$($PSQL "SELECT (SUM(CAST(winner_goals AS DECIMAL)) + SUM(opponent_goals))/COUNT(*) from games;")"
